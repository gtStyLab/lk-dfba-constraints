function [optimalParams,solverStatus,solverPopulation,hyperplaneParams] = fitLKDFBA_UpDownReg_HP(odeTimeVec,odeConcMatrix,odeFluxMatrix,modelStruct,dataWeightVector,fitOptions)
...
% Edits to use hyperplane (HP) constraints JL2020
    ...
    if ~isfield(fitOptions,'pLB')
        fitOptions.pLB = -inf(numUniq+numSconstraints,2);
    end
    if ~isfield(fitOptions,'pUB')
        fitOptions.pUB = inf(numUniq+numSconstraints,2);
    end
    
    paramUBs = fitOptions.pUB;
    paramLBs = fitOptions.pLB;
    
    if size(paramUBs,1) ~= numUniq + numSconstraints
        paramUBs = paramUBs';
        paramLBs = paramLBs';
    end
    ...
    elseif size(paramLBs,1) ~= numUniq + numSconstraints
    ...
    if ~isfield(fitOptions,'fixedParameterSequence')
        fitOptions.fixedParameterSequence = false(numUniq + numSconstraints,1);
    else
    ...
    guessedParams = false(numUniq + numSconstraints,1);
    ...
    % Otherwise, let's do our typical regression routine.
        ...
        linearParamsMatrix = nan(numUniq + numSconstraints,2);
        ...
        for n = 1:numUniq
            ...
            if all(paramLBs(n,:) == paramUBs(n,:)) && all(~isinf(paramLBs(n,:)))
                ...
                if isempty(xData)
                    ...
                    hyperplaneParams{n} = 0;
                %{ 
                % BREAKS IF ONLY ONE DATA POINT JL20Feb20
                elseif all(xData==xData(1))
                    ...
                    if paramUBs(n,1) <= 0 && paramLBs(n,1) < 0
                        linearParamsMatrix(n,:) = [max([-0.01,paramLBs(n,1)]),max(sum(odeFluxMatrix(2:end,modelStruct.kineticsMap{fluxIdx,2}),2)) + 0.01 * odeConcMatrix(1,modelStruct.kineticsMap{fluxIdx,1})];
                    else
            ...
            v0 = linearParamsMatrix(n,1) * fitProblemStruct.x0(modelStruct.HPkineticsMap{n,1}) + linearParamsMatrix(n,2);
            if v0 < sum(modelStruct.vBounds(modelStruct.HPkineticsMap{n,2},1))
        ...
        for n = numUniq+1:numUniq+numSconstraints
            if all(paramLBs(n,:) == paramUBs(n,:)) && all(~isinf(paramLBs(n,:)))
                % We have non-inf pre-specified values --> don't fit
                linearParamsMatrix(n,:) = paramLBs(n,:);
                hyperplaneParams{n} = 1;
                ...
                % We're regressing fluxes (y) against concentrations (x)
                xData = controllerMet_combined{n-numUniq};
                ...
                elseif all(xData==xData(1))
                    ...
                    % and let the solver find better numbers later.
                    if paramUBs(n,1) <= 0 && paramLBs(n,1) < 0
                        linearParamsMatrix(n,:) = [max([-0.01,paramLBs(n,1)]),max(sum(odeFluxMatrix(2:end,modelStruct.kineticsMap{dupIdx(n-numUniq),2}),2)) + 0.01 * odeConcMatrix(1,modelStruct.kineticsMap{dupIdx(n-numUniq),1})];
                    else
                        linearParamsMatrix(n,:) = [min([ 0.01,paramUBs(n,1)]),max(sum(odeFluxMatrix(2:end,modelStruct.kineticsMap{dupIdx(n-numUniq),2}),2))];
                    end
                    guessedParams(n) = true;
                %}
                else
                    % Other we'll just fit with the linear approximation.
                    f = fitlm(xData,yData);
                    linearParamsMatrix(n,1) = 1;
                    linearParamsMatrix(n,2) = f.Coefficients.Estimate(1);
                    hyperplaneParams{n} = f.Coefficients.Estimate(2:end);
                    
                end
            end
            
            v0 = sum(fitProblemStruct.x0(modelStruct.HPkineticsMap{n,1}).*hyperplaneParams{n}') + linearParamsMatrix(n,2);
            if v0 < sum(modelStruct.vBounds(modelStruct.HPkineticsMap{n,2},1))
                % If this happens, we need to adjust b so that v(x0) is
                % a valid flux value, i.e. it exceeds the (sum of the)
                % lower bounds of the target flux. If reversible
...
% %     fbaProblemStruct.fitOptions.penaltyWeight = fitOptions.penaltyWeight;
    ...
    if strcmp(fitOptions.solverType,'ga') ||  strcmp(fitOptions.solverType,'fmincon')
        ...
        A_ineq = zeros(2*(numUniq+numSconstraints),2*(numUniq+numSconstraints));
        b_ineq = zeros(2*(numUniq+numSconstraints),1);

        % Add b_i-constraints for when a_i < 0 --> Potentially infeasible
        A_ineq(1:numUniq+numSconstraints,:) = -[diag(fitProblemStruct.x0([modelStruct.kineticsMap{:,1}])),eye(numUniq+numSconstraints)];
        for i = 1:numUniq+numSconstraints
        ...
        for i = 1:numUniq+numSconstraints
            ...
            if idxPos(i) && ~idxNeg(i) && ~isinf(min(modelStruct.vBounds(modelStruct.kineticsMap{i,2},2)))
                A_ineq(numUniq+numSconstraints+i,numUniq+numSconstraints+i) = 1;
                b_ineq(numUniq+numSconstraints+i) = min(modelStruct.vBounds(modelStruct.kineticsMap{i,2},2));
            end
            ...
            if idxNeg(i) && ~idxPos(i) && ~isinf(max(modelStruct.vBounds(modelStruct.kineticsMap{i,2},1)))
                A_ineq(numUniq+numSconstraints+i,numUniq+numSconstraints+i) = -1;
                b_ineq(numUniq+numSconstraints+i) = -max(modelStruct.vBounds(modelStruct.kineticsMap{i,2},1));
            end 
        end
    end
    ...
    if fitOptions.nTries == 0 || isempty(fitOptions.fixedParameterSequence)
        ...
        fixedList = false(numUniq+numSconstraints,1);
        ...
        solverStatus.fval = calcFcnErrWrapper(optimalParams,fitProblemStruct,fixedParams,hyperplaneParams);
    else
                ...
                for i = 1:fitOptions.nTries
                    ...
                    while ~successFlag
                        ...
                        try
                            ...
                            [~,~,~,testStatus] = calcLKDFBA_HP(fitProblemStruct.tStart,...
                                                         ...
                                                         fitProblemStruct.modelStruct.HPkineticsMap,...
                                                         testParams,...
                                                         fitProblemStruct.modelStruct,hyperplaneParams);
                        catch
...
function totalError = calcFcnErrWrapper(fitParams,paramStruct,fixedParams,hyperplaneParams)
...
% search space when I want to pre-specify parameters.
    ...
    if isfield(paramStruct,'fitnessFunction') && ~isempty(paramStruct.fitnessFunction) && isa(paramStruct.fitnessFunctionName,'string') && exist(sprintf('%s.m',paramStruct.fitnessFunctionName),'file')
        ...
        totalError = defaultCalcFcnErr(functionParams,paramStruct,hyperplaneParams);
    end
    
end

function totalError = defaultCalcFcnErr(fitParams,paramStruct,hyperPlane)
...
% specified weight vector.
    ...
    [timeVec,fbaConcMatrix,fbaFluxMatrix,result] = ...
                                calcLKDFBA_HP(paramStruct.tStart,...
                                        paramStruct.tEnd,...
                                        nT,...
                                        paramStruct.x0,...
                                        ...
                                        paramStruct.modelStruct.HPkineticsMap,...
                                        regulationParams,...
                                        paramStruct.modelStruct,hyperPlane);
    ...
    if strcmp(result.status,'OPTIMAL') % || strcmp(result.status,'SUBOPTIMAL')
        ...
        %interpConcMatrix = interp1(timeVec,fbaConcMatrix,odeTimeVec(1:paramStruct.nT),'linear','extrap');
...
function [errorVector] = calcFcnErrVector(fitParams,paramStruct,hyperplaneParams)
% This is what we're throwing in the solver when we optimize parameters
% It does some formatting/bookkeeping, calls the FBA routine, then
% calculates our error relative to the training data, subject to the
% specified weight vector.
    ...
    [timeVec,fbaConcMatrix,fbaFluxMatrix,result] = ...
                                calcLKDFBA_HP(paramStruct.tStart,...
                                        ...
                                        paramStruct.modelStruct.HPkineticsMap,...
                                        regulationParams,...
                                        paramStruct.modelStruct,hyperplaneParams);
    fluxTimeVec = timeVec(1:end-1)+0.5*diff(timeVec(1:2));
