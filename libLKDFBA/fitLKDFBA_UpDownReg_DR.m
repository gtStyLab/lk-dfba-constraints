function [optimalParams,solverStatus,solverPopulation,coeff,modelStruct] = fitLKDFBA_UpDownReg_DR(odeTimeVec,odeConcMatrix,odeFluxMatrix,modelStruct,dataWeightVector,fitOptions)
...
% Edits to use dimension reduction (DR) constraints JL2020
    ...
    %Might not be correct for future models...make sure indexing of
    %kineticsMap is correct later on in LR calculation
    
    for idx = 1:length(uniqIdx)
        newLBs(idx,1:2) = fitOptions.pLB(find([modelStruct.kineticsMap{:,2}] == fluxList(uniqIdx(idx))),:);
        newUBs(idx,1:2) = fitOptions.pUB(find([modelStruct.kineticsMap{:,2}] == fluxList(uniqIdx(idx))),:);
        modelStruct.DRkineticsMap{idx,1} = modelStruct.kineticsMap{find([modelStruct.kineticsMap{:,2}] == fluxList(uniqIdx(idx))),1};
        modelStruct.DRkineticsMap{idx,2} = modelStruct.kineticsMap{find([modelStruct.kineticsMap{:,2}] == fluxList(uniqIdx(idx))),2};
        modelStruct.DRkineticsMap{idx,3} = modelStruct.kineticsMap{find([modelStruct.kineticsMap{:,2}] == fluxList(uniqIdx(idx))),3};
    end
    ...
    for idx = 1:length(dupIdx)
        ...
        controllerMet_combined{idx} = [];
        for numDup = 1:length(sameFluxIdx)
            controllerMet_combined{idx} = [controllerMet_combined{idx} odeConcMatrix(:,modelStruct.kineticsMap{sameFluxIdx(numDup),1})];
        end

        controllerMet_combined_standard = (controllerMet_combined{idx});
        [coeff{idx}] = pca(controllerMet_combined_standard,'Centered','off');
        
        newLBs(idx+numUniq,1:2) = [-inf -inf];
        newUBs(idx+numUniq,1:2) = [inf inf];
        modelStruct.DRkineticsMap{idx+numUniq,1} = [modelStruct.kineticsMap{find([modelStruct.kineticsMap{:,2}] == fluxList(dupIdx(idx))),1}];
        modelStruct.DRkineticsMap{idx+numUniq,2} = modelStruct.kineticsMap{find([modelStruct.kineticsMap{:,2}] == fluxList(dupIdx(idx))),2};
        modelStruct.DRkineticsMap{idx+numUniq,3} = modelStruct.kineticsMap{find([modelStruct.kineticsMap{:,2}] == fluxList(dupIdx(idx))),3};
    end
    
    numDRconstraints = length(dupIdx);
    
    % Make sure we have a LB and UB for parameters defined
    
    fitOptions.pLB = newLBs;
    fitOptions.pUB = newUBs;
    
    if ~isfield(fitOptions,'pLB')
        fitOptions.pLB = -inf(numUniq+numDRconstraints,2);
    end
    if ~isfield(fitOptions,'pUB')
        fitOptions.pUB = inf(numUniq+numDRconstraints,2);
    end
    
    paramUBs = fitOptions.pUB;
    paramLBs = fitOptions.pLB;
    
    if size(paramUBs,1) ~= numUniq + numDRconstraints
        paramUBs = paramUBs';
        paramLBs = paramLBs';
    end
    ...
    elseif size(paramLBs,1) ~= numUniq + numDRconstraints
    ...
    if ~isfield(fitOptions,'fixedParameterSequence')
        fitOptions.fixedParameterSequence = false(numUniq + numDRconstraints,1);
    else
    ...
    guessedParams = false(numUniq + numDRconstraints,1);
    ...
    % Otherwise, let's do our typical regression routine.
        ...
        linearParamsMatrix = nan(numUniq + numDRconstraints,2);

        % For fluxes with only one controller metabolite JL20Mar18
        numUniq = length(uniqIdx);
        for n = 1:numUniq
            ...
            if all(paramLBs(n,:) == paramUBs(n,:)) && all(~isinf(paramLBs(n,:)))
                ...
                fluxIdx = find([modelStruct.kineticsMap{:,2}] == fluxList(uniqIdx(n)));
                xData = odeConcMatrix(:,modelStruct.kineticsMap{fluxIdx,1});
                yData = sum(odeFluxMatrix(:,modelStruct.kineticsMap{fluxIdx,2}),2);

                idx0 = find((yData<=0)); % Don't include negative flux values (which should have been moved to another reverse flux) JL02Jan20
                xData(idx0) = [];
                yData(idx0) = [];
                
                idxNaN = find(isnan(yData));
                xData(idxNaN) = [];
                yData(idxNaN) = [];
                ...
                elseif all(xData==xData(1))
                    ...
                    if paramUBs(n,1) <= 0 && paramLBs(n,1) < 0
                        linearParamsMatrix(n,:) = [max([-0.01,paramLBs(n,1)]),max(sum(odeFluxMatrix(2:end,modelStruct.kineticsMap{fluxIdx,2}),2)) + 0.01 * odeConcMatrix(1,modelStruct.kineticsMap{uniqIdx(n),1})];
                    else
                        linearParamsMatrix(n,:) = [min([ 0.01,paramUBs(n,1)]),max(sum(odeFluxMatrix(2:end,modelStruct.kineticsMap{fluxIdx,2}),2))];
                    end
                    guessedParams(n) = true;
                else
                    % Other we'll just fit with the linear approximation.
                    findNotNaN = find(~isnan(yData));
                    xData = xData(findNotNaN);
                    yData = yData(findNotNaN);
                    
                    linearParamsMatrix(n,:) = polyfit(xData,yData,1);
                end
            end
            
            v0 = linearParamsMatrix(n,1) * fitProblemStruct.x0(modelStruct.DRkineticsMap{n,1}) + linearParamsMatrix(n,2);
            if v0 < sum(modelStruct.vBounds(modelStruct.DRkineticsMap{n,2},1))
        ...
        % Dimension Reduction JL20Mar18
        % For fluxes with multiple controller metabolites
        for n = numUniq+1:numUniq+numDRconstraints
            if all(paramLBs(n,:) == paramUBs(n,:)) && all(~isinf(paramLBs(n,:)))
                % We have non-inf pre-specified values --> don't fit
                linearParamsMatrix(n,:) = paramLBs(n,:);
            else
                ...
                % We're regressing fluxes (y) against concentrations (x)
                xData = controllerMet_combined{n-numUniq}*coeff{n-numUniq}(:,1);
                xData = xData(:,1);

                yData = sum(odeFluxMatrix(:,fluxList(dupIdx(n-numUniq))),2);
                ...
                elseif all(xData==xData(1))
                    ...
                    % and let the solver find better numbers later.
                    if paramUBs(n,1) <= 0 && paramLBs(n,1) < 0
                        linearParamsMatrix(n,:) = [max([-0.01,paramLBs(n,1)]),max(sum(odeFluxMatrix(2:end,fluxList(dupIdx(n-numUniq))),2)) + 0.01 * odeConcMatrix(1,modelStruct.kineticsMap{dupIdx(n-numUniq),1})];
                    else
                        linearParamsMatrix(n,:) = [min([ 0.01,paramUBs(n,1)]),max(sum(odeFluxMatrix(2:end,fluxList(dupIdx(n-numUniq))),2))];
                    end
                    guessedParams(n) = true;
                else
                    % Other we'll just fit with the linear approximation.
            ...
            v0 = linearParamsMatrix(n,1) * fitProblemStruct.x0(modelStruct.DRkineticsMap{n,1})*coeff{n-numUniq}(:,1) + linearParamsMatrix(n,2);
            if v0 < sum(modelStruct.vBounds(modelStruct.DRkineticsMap{n,2},1))
                % If this happens, we need to adjust b so that v(x0) is
                % a valid flux value, i.e. it exceeds the (sum of the)
                % lower bounds of the target flux. If reversible
...
% %     fbaProblemStruct.fitOptions.penaltyWeight = fitOptions.penaltyWeight;
    ...
    if strcmp(fitOptions.solverType,'ga') ||  strcmp(fitOptions.solverType,'fmincon')
        ...
        A_ineq = zeros(2*(numUniq+numDRconstraints),2*(numUniq+numDRconstraints));
        b_ineq = zeros(2*(numUniq+numDRconstraints),1);

        % Add b_i-constraints for when a_i < 0 --> Potentially infeasible
        A_ineq(1:numUniq+numDRconstraints,:) = -[diag(fitProblemStruct.x0([modelStruct.kineticsMap{:,1}])),eye(numUniq+numDRconstraints)];
        for i = 1:numUniq+numDRconstraints
        ...
        for i = 1:numUniq+numDRconstraints
            ...
            if idxPos(i) && ~idxNeg(i) && ~isinf(min(modelStruct.vBounds(modelStruct.kineticsMap{i,2},2)))
                A_ineq(numUniq+numDRconstraints+i,numUniq+numDRconstraints+i) = 1;
                b_ineq(numUniq+numDRconstraints+i) = min(modelStruct.vBounds(modelStruct.kineticsMap{i,2},2));
            end
            ...
            if idxNeg(i) && ~idxPos(i) && ~isinf(max(modelStruct.vBounds(modelStruct.kineticsMap{i,2},1)))
                A_ineq(numUniq+numDRconstraints+i,numUniq+numDRconstraints+i) = -1;
                b_ineq(numUniq+numDRconstraints+i) = -max(modelStruct.vBounds(modelStruct.kineticsMap{i,2},1));
            end 
        end
    end
    ...
    if fitOptions.nTries == 0 || isempty(fitOptions.fixedParameterSequence)
        ...
        fixedList = false(numUniq+numDRconstraints,1);
        fixedParams = optimalParams;
        fixedParams(~fixedList,:) = nan;
        solverStatus = struct('flag','Regression','fval', 'NaN','guessedParams',guessedParams);
        solverStatus.fval = calcFcnErrWrapper(optimalParams,fitProblemStruct,fixedParams,coeff);
    else
        ...
        for j = 1:size(fixedMatrix,2)
            ...
            if isfield(fitOptions,'useHeuristicWeights') && fitOptions.useHeuristicWeights
                ...
                errorVector = sqrt(abs(calcFcnErrVector(currentParamsMatrix,fitProblemStruct)),coeff);
            ...
            if  strcmp(fitOptions.solverType,'ga')
                ...
                % Solve with GA
                [solverX,solverFval,solverFlag,solverStatusj,solverPopulationj] = ga(@(x) calcFcnErrWrapper(x,fitProblemStruct,fixedParams,coeff),numel(initParams),A_ineq_j,b_ineq_j,[],[],pLB,pUB,[],[],eval(optionsString));
                ...
                for i = 1:fitOptions.nTries
                    ...
                    while ~successFlag
                        ...
                        try
                            ...
                            [~,~,~,testStatus] = calcLKDFBA_DR(fitProblemStruct.tStart,...
                                                         ...
                                                         testParams,...
                                                         fitProblemStruct.modelStruct,coeff);
                        catch
                        ...
                        if strcmp(testStatus.status,'OPTIMAL')
                            ...
                            try
                                if strcmp(fitOptions.solverType,'fmincon')
                                    [solverX,solverFval,solverFlag,solverStatusi] = fmincon(@(x) calcFcnErrWrapper(x,fitProblemStruct,fixedParams,coeff),initParams(:)',A_ineq_j,b_ineq_j,[],[],pLB,pUB,[],optimoptions('fmincon','Display',fitOptions.fitProgressDisplay));
                                else
                                    [solverX,solverFval,solverFlag,solverStatusi] = fminsearch(@(x) calcFcnErrWrapper(x,fitProblemStruct,fixedParams,coeff),initParams(:)',optimset('Display',fitOptions.fitProgressDisplay));
                                end
...
function totalError = calcFcnErrWrapper(fitParams,paramStruct,fixedParams,PCAcoeff)
...
% search space when I want to pre-specify parameters.
    ...
    if isfield(paramStruct,'fitnessFunction') && ~isempty(paramStruct.fitnessFunction) && isa(paramStruct.fitnessFunctionName,'string') && exist(sprintf('%s.m',paramStruct.fitnessFunctionName),'file')
        ...
        totalError = defaultCalcFcnErr(functionParams,paramStruct,PCAcoeff);
    end
    
end

function totalError = defaultCalcFcnErr(fitParams,paramStruct,PCAcoeff)
...
% specified weight vector.
    ...
    [timeVec,fbaConcMatrix,fbaFluxMatrix,result] = ...
                                calcLKDFBA_DR(paramStruct.tStart,...
                                        paramStruct.tEnd,...
                                        nT,...
                                        paramStruct.x0,...
                                        ...
                                        regulationParams,...
                                        paramStruct.modelStruct,PCAcoeff);
    ...
    if strcmp(result.status,'OPTIMAL') % || strcmp(result.status,'SUBOPTIMAL')
        ...
        interpConcMatrix = interp1(timeVec,fbaConcMatrix,odeTimeVec(1:paramStruct.nT+1),'linear','extrap');
        %^ Changed paramStruct.nT to paramStruct.nT+1 JL21Feb19
...
function [errorVector] = calcFcnErrVector(fitParams,paramStruct,PCAcoeff)
% This is what we're throwing in the solver when we optimize parameters
% It does some formatting/bookkeeping, calls the FBA routine, then
% calculates our error relative to the training data, subject to the
% specified weight vector.
    ...
    if ~isfield(paramStruct,'odeFluxTimeVec')
         if length(odeTimeVec) == size(paramStruct.odeFluxMatrix,1)
             odeFluxTimeVec = odeTimeVec;
         else % length(odeTimeVec) == size(paramStruct.odeFluxMatrix,1) + 1
            odeFluxTimeVec = odeTimeVec(1:end-1)+0.5*diff(odeTimeVec(1:2));
         end
    end
