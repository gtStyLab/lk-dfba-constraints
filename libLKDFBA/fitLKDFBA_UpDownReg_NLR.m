function [optimalParams,solverStatus,solverPopulation] = fitLKDFBA_UpDownReg_NLR(odeTimeVec,odeConcMatrix,odeFluxMatrix,modelStruct,dataWeightVector,fitOptions)
...
% Edits to use nonlinear regression (NLR) constraints JL2020
    ...
    if isfield(fitOptions,'seedParameters') && ~isempty(fitOptions.seedParameters)
        quadParamsMatrix = fitOptions.seedParameters;
    ...
    % Otherwise, let's do our typical regression routine.
        ...
        % y=a*x^2+b*x+c to find regulatory a, b, and c parameters.
        quadParamsMatrix = zeros(numRegs,3);

        for n = 1:numRegs
            
            if all(paramLBs(n,:) == paramUBs(n,:)) && all(~isinf(paramLBs(n,:)))
                % We have non-inf pre-specified values --> don't fit
                quadParamsMatrix(n,2) = paramLBs(n,1);
            else

                % We're regressing fluxes (y) against concentrations (x)
                xData = odeConcMatrix(:,modelStruct.kineticsMap{n,1});
                yData = sum(odeFluxMatrix(:,modelStruct.kineticsMap{n,2}),2);
                ...
                if isempty(xData)
                    ...
                    quadParamsMatrix(n,:) = [0,0,0];
                %{
                Ignore this scenario JL07Jan19
                elseif all(xData==xData(1))
                    ...
                    if paramUBs(n,1) <= 0 && paramLBs(n,1) < 0
                        quadParamsMatrix(n,:) = [max([-0.01,paramLBs(n,1)]),max(sum(odeFluxMatrix(2:end,modelStruct.kineticsMap{n,2}),2)) + 0.01 * odeConcMatrix(1,modelStruct.kineticsMap{n,1})];
                    else
                        quadParamsMatrix(n,:) = [min([ 0.01,paramUBs(n,1)]),max(sum(odeFluxMatrix(2:end,modelStruct.kineticsMap{n,2}),2))];
                    end
                    guessedParams(n) = true;
                %}
                else
                    % Otherwise we'll just fit with the quadratic
                    % approximation.
                    quadParamsMatrix(n,:) = polyfit(xData,yData,2);
                    
                    % If quadratic fit is not convex or some params are too
                    % negative or too large, revert to linear constraints
                    if quadParamsMatrix(n,1) > 0 || quadParamsMatrix(n,1) < -1e12 || abs(quadParamsMatrix(n,2)) > 1e12
                        quadParamsMatrix(n,1) = 0;
                        quadParamsMatrix(n,2:3) = polyfit(xData,yData,1);
                    end
                end
            end
            
            for k = 1:length(modelStruct.kineticsMap{n,1})
                v0 = quadParamsMatrix(n,1) * fitProblemStruct.x0(modelStruct.kineticsMap{n,1}(k))^2 + quadParamsMatrix(n,2) * fitProblemStruct.x0(modelStruct.kineticsMap{n,1}(k)) + quadParamsMatrix(n,3);
                if v0 < sum(modelStruct.vBounds(modelStruct.kineticsMap{n,2},1))
                    ...
                    % always be zero.
                    quadParamsMatrix(n,3) = quadParamsMatrix(n,3) - (1 + vx0BufferFraction)*v0;
                end
            end
    ...
    if fitOptions.useNonzeroRegularization
        ...
        fitProblemStruct.referenceParams = quadParamsMatrix;
        ...
        fitProblemStruct.referenceParams = zeros(size(quadParamsMatrix));
...
% %     fbaProblemStruct.fitOptions.penaltyWeight = fitOptions.penaltyWeight;
    ...
    if fitOptions.nTries == 0 || isempty(fitOptions.fixedParameterSequence)
        ...
        % procedure.
        optimalParams =  quadParamsMatrix;
        ...
        currentParamsMatrix = quadParamsMatrix;
                ...
                for i = 1:fitOptions.nTries
                    ...
                    while ~successFlag
                        ...
                        try
                            ...
                            [~,~,~,testStatus] = calcLKDFBA_NLR(fitProblemStruct.tStart,...
...
% specified weight vector.
    ...
    [timeVec,fbaConcMatrix,fbaFluxMatrix,result] = ...
                                calcLKDFBA_NLR(paramStruct.tStart,...
                                        paramStruct.tEnd,...
                                        nT,...
                                        paramStruct.x0,...
