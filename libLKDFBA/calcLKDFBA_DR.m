function [timeVec,concMatrix,fluxMatrix,solution] = calcLKDFBA_DR(tStart,tEnd,nT,x0,STM,xBounds,vBounds,kineticsMap,kineticsParams,runParams,PCAcoeff)
...
% improvements.
% Edited JL2020 for dimension reduction (DR) constraints.
    ...
    numDRconstraints = numel(PCAcoeff);
    numRegs = length(uniqIdx);
    ...
    L3 = nT*(numRegs+numDRconstraints); % Number of Regulatory Constraints
    ...
    if size(kineticsMap,2) == 3
        regSense = repmat([runParams.DRkineticsMap{:,3}],1,nT)';
    else
    ...
    if numRegs ~= 0
        [model.A(L1+L2+1:L1+L2+L3, 1:nT*(numAugmentedFluxes+numMetabs)), model.rhs(L1+L2+1:L1+L2+L3)] = ...
            buildRegulatoryIneqConstraints(kineticsMap,kineticsParams,numMetabs,numAugmentedFluxes,nT,PCAcoeff);
    end
...
function [outputMatrix, rhs] = buildRegulatoryIneqConstraints(kineticsMap,regulationParams,numMetabs,numAugmentedFluxes,numBlocks,PCAcoeff)
...
% Written 2015-09-02 by R.A.Dromms
    ...
    controllerMatrix = spalloc(numRegs+numDRconstraints,numMetabs,numRegs+numDRconstraints);
    for i = 1:numRegs
        fluxIdx = find([kineticsMap{:,2}] == fluxList(uniqIdx(i)));
        controllerMatrix(i,kineticsMap{fluxIdx,1}) = -regulationParams(i,1);
    end
    for i = numRegs+1:numRegs+numDRconstraints
        sameFluxIdx = find([kineticsMap{:,2}] == fluxList(dupIdx(i-numRegs)));
        controllerMatrix(i,[kineticsMap{sameFluxIdx,1}]) = [-regulationParams(i,1)*PCAcoeff{i-numRegs}(:,1)'];
    end
    
    % 2) Construct target matrix
    targetMatrix = spalloc(numRegs+numDRconstraints,numAugmentedFluxes,numRegs+numDRconstraints);
    for i = 1:numRegs
        targetMatrix(i,fluxList(uniqIdx(i))) = 1;
    end
    for i = numRegs+1:numRegs+numDRconstraints
        targetMatrix(i,fluxList(dupIdx(i-numRegs))) = 1;
    end
