function [timeVec,concMatrix,fluxMatrix,solution] = calcLKDFBA_HP(tStart,tEnd,nT,x0,STM,xBounds,vBounds,HPkineticsMap,kineticsParams,runParams,hyperplaneParams)
...
% Edited by R.A. Dromms from 2015-Sept to 2017-June for various tweaks and
% improvements.
% Edited JL2020 for hyperplane constraints.
    ...
    [fluxList] = unique([runParams.kineticsMap{:,2}],'stable');
    
    countDup = [];
    for idx = 1:length(fluxList)
        countDup = [countDup sum(length(find([runParams.kineticsMap{:,2}]==fluxList(idx))))];
    end
    ...
    numSconstraints = numel(hyperplaneParams) - numUniq;
    ...
    L3 = nT*(numUniq+numSconstraints); % Number of Regulatory Constraints
    ...
    if size(HPkineticsMap,2) == 3
        regSense = repmat([runParams.HPkineticsMap{:,3}],1,nT)';
    else
    ...
    if numUniq ~= 0
        [model.A(L1+L2+1:L1+L2+L3, 1:nT*(numAugmentedFluxes+numMetabs)), model.rhs(L1+L2+1:L1+L2+L3)] = ...
            buildRegulatoryIneqConstraints(runParams.kineticsMap,kineticsParams,numMetabs,numAugmentedFluxes,nT,hyperplaneParams);
    end
    ...
    solution = gurobi(model, gurobiParams);
    solution.timeVec = timeVec;
...
function [outputMatrix, rhs] = buildRegulatoryIneqConstraints(kineticsMap,regulationParams,numMetabs,numAugmentedFluxes,numBlocks,hyperplaneParams)
...
% Written 2015-09-02 by R.A.Dromms
    ...
    % Surface fit JL13Mar18
    [fluxList] = unique([kineticsMap{:,2}],'stable');
    
    countDup = [];
    for idx = 1:length(fluxList)
        countDup = [countDup sum(length(find([kineticsMap{:,2}]==fluxList(idx))))];
    end
    ...
    controllerMatrix = spalloc(numUniq+numSconstraints,numMetabs,numUniq+numSconstraints);
    for i = 1:numUniq
        fluxIdx = find([kineticsMap{:,2}] == fluxList(uniqIdx(i)));
        controllerMatrix(i,kineticsMap{fluxIdx,1}) = -regulationParams(i,1)*hyperplaneParams{i};
    end
    for i = numUniq+1:numUniq+numSconstraints
        sameFluxIdx = find([kineticsMap{:,2}] == fluxList(dupIdx(i-numUniq)));
        controllerMatrix(i,[kineticsMap{sameFluxIdx,1}]) = [-regulationParams(i,1)*hyperplaneParams{i}];
    end
    
    % 2) Construct target matrix
    targetMatrix = spalloc(numUniq+numSconstraints,numAugmentedFluxes,numUniq+numSconstraints);
    for i = 1:numUniq
        targetMatrix(i,fluxList(uniqIdx(i))) = 1;
    end
    for i = numUniq+1:numUniq+numSconstraints
        targetMatrix(i,fluxList(dupIdx(i-numUniq))) = 1;
    end
