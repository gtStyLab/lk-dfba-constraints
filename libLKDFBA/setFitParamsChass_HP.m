% This script sets default hyperplane fitting parameters for the E. coli
% model. Default values contained here are be overwritten by the wrapper 
...
for SFIdx = 1:numSconstraints
    modelStruct.HPkineticsMap{numUniq+SFIdx,1} = [modelStruct.kineticsMap{find([modelStruct.kineticsMap{:,2}] == fluxList(dupIdx(SFIdx))),1}];
    modelStruct.HPkineticsMap{numUniq+SFIdx,2} = fluxList(dupIdx(SFIdx));
    modelStruct.HPkineticsMap{numUniq+SFIdx,3} = '<';
end

fitOptions.pLB(1:17,1) = 2.78e-05;
fitOptions.pUB(1:17,1) = 2.78e-05;
fitOptions.pLB(1:17,2) = 0;
fitOptions.pUB(1:17,2) = 0;
