% Predict down/up regulations in noisy E. coli model 

clc; clear; close all

% Choose flux to down/up regulate
regName = ''; % dPYK, dPGI, dGAPDH, dPFK, dTIS, dRPE, dPGM
            % dG6PDH dPGDH dALDO dPGluMu dR5PI dTKa dTKb dTA

refDataName = 'chassV_k-01_hiRes.mat';
dataDir = '';
nTErrorCalc = 100;
...
refData.fluxMatrix = refData.fluxMatrix(:,1:48);
load('chassV_k-01_hiRes.mat','concNames','fluxNames');
refData.concNames = concNames;
refData.fluxNames = fluxNames;

load('chassSTM_rev.mat','stm')
refData.STM = stm;
...
for k = 1:length(fileNameList)
    ...
    if exist(fileName,'file')
        ...
        data = load(fileName);
        data.nTFit = 50;
        data.modelStruct.objectiveFluxIndex = [10 15 17 21 22 23 24 25 30 31];
        ...
        if ~any(strcmp(fileName,excludeList))
            ...
            if ~isempty(strfind(fileName,'fbaRegressionPlusParams.mat')) || ~isempty(strfind(fileName,'fbaRegressionParams.mat')) || ~isempty(strfind(fileName,'fbaGaParams.mat'))
                ...
                if strcmp(regName,'dPYK')
                    % Downregulate PYK
                    params_location = find([tempKineticsMap{:,2}] == 20);
                    data.optimalParams(params_location,:) = 0.1*data.optimalParams(params_location,:);
                elseif strcmp(regName,'dPGI')
                    % Downregulate PGI
                    params_location = find([tempKineticsMap{:,2}] == 3);
                    data.optimalParams(params_location,:) = 0.1*data.optimalParams(params_location,:);
                    params_location = find([tempKineticsMap{:,2}] == 49);
                    data.optimalParams(params_location,:) = 0.1*data.optimalParams(params_location,:);
                elseif strcmp(regName,'dGAPDH')
                    % Downregulate GAPDH
                     params_location = find([tempKineticsMap{:,2}] == 12);
                    data.optimalParams(params_location,:) = 0.1*data.optimalParams(params_location,:);
                    params_location = find([tempKineticsMap{:,2}] == 55);
                    data.optimalParams(params_location,:) = 0.1*data.optimalParams(params_location,:);
                elseif strcmp(regName,'dPFK')
                    % Downregulate PFK
                     params_location = find([tempKineticsMap{:,2}] == 6);
                    data.optimalParams(params_location,:) = 0.1*data.optimalParams(params_location,:);
                elseif strcmp(regName,'dTIS')
                    % Downregulate TIS
                     params_location = find([tempKineticsMap{:,2}] == 13);
                    data.optimalParams(params_location,:) = 0.1*data.optimalParams(params_location,:);
                    params_location = find([tempKineticsMap{:,2}] == 56);
                    data.optimalParams(params_location,:) = 0.1*data.optimalParams(params_location,:);
                elseif strcmp(regName,'dRPE')
                    % Downregulate RPE
                     params_location = find([tempKineticsMap{:,2}] == 29);
                    data.optimalParams(params_location,:) = 0.1*data.optimalParams(params_location,:);
                    params_location = find([tempKineticsMap{:,2}] == 61);
                    data.optimalParams(params_location,:) = 0.1*data.optimalParams(params_location,:);
                elseif strcmp(regName,'dPGM')
                    % Downregulate PGM
                     params_location = find([tempKineticsMap{:,2}] == 4);
                    data.optimalParams(params_location,:) = 0.1*data.optimalParams(params_location,:);
                    params_location = find([tempKineticsMap{:,2}] == 50);
                    data.optimalParams(params_location,:) = 0.1*data.optimalParams(params_location,:);
                elseif strcmp(regName,'dG6PDH')
                    % Downregulate G6PDH (zwf)
                     params_location = find([tempKineticsMap{:,2}] == 5);
                    data.optimalParams(params_location,:) = 0.1*data.optimalParams(params_location,:);
                elseif strcmp(regName,'dPGDH')
                    % Downregulate PGDH (gnd)
                     params_location = find([tempKineticsMap{:,2}] == 27);
                    data.optimalParams(params_location,:) = 0.1*data.optimalParams(params_location,:);
                elseif strcmp(regName,'dALDO')
                    % Downregulate ALDO (fbaB)
                     params_location = find([tempKineticsMap{:,2}] == 11);
                    data.optimalParams(params_location,:) = 0.1*data.optimalParams(params_location,:);
                    params_location = find([tempKineticsMap{:,2}] == 54);
                    data.optimalParams(params_location,:) = 0.1*data.optimalParams(params_location,:);
                elseif strcmp(regName,'dPGluMu')
                    % Downregulate PGluMu (gpmB)
                     params_location = find([tempKineticsMap{:,2}] == 18);
                    data.optimalParams(params_location,:) = 0.1*data.optimalParams(params_location,:);
                    params_location = find([tempKineticsMap{:,2}] == 58);
                    data.optimalParams(params_location,:) = 0.1*data.optimalParams(params_location,:);
                elseif strcmp(regName,'dR5PI')
                    % Downregulate R5PI (rpiB)
                     params_location = find([tempKineticsMap{:,2}] == 28);
                    data.optimalParams(params_location,:) = 0.1*data.optimalParams(params_location,:);
                    params_location = find([tempKineticsMap{:,2}] == 60);
                    data.optimalParams(params_location,:) = 0.1*data.optimalParams(params_location,:);
                elseif strcmp(regName,'dTKa')
                    % Downregulate TKa (tktA)
                     params_location = find([tempKineticsMap{:,2}] == 8);
                    data.optimalParams(params_location,:) = 0.1*data.optimalParams(params_location,:);
                    params_location = find([tempKineticsMap{:,2}] == 52);
                    data.optimalParams(params_location,:) = 0.1*data.optimalParams(params_location,:);
                elseif strcmp(regName,'dTKb')
                    % Downregulate TKb (tktB)
                     params_location = find([tempKineticsMap{:,2}] == 9);
                    data.optimalParams(params_location,:) = 0.1*data.optimalParams(params_location,:);
                    params_location = find([tempKineticsMap{:,2}] == 53);
                    data.optimalParams(params_location,:) = 0.1*data.optimalParams(params_location,:);
                elseif strcmp(regName,'dTA')
                    % Downregulate TA (talB)
                     params_location = find([tempKineticsMap{:,2}] == 7);
                    data.optimalParams(params_location,:) = 0.1*data.optimalParams(params_location,:);
                    params_location = find([tempKineticsMap{:,2}] == 51);
                    data.optimalParams(params_location,:) = 0.1*data.optimalParams(params_location,:);
